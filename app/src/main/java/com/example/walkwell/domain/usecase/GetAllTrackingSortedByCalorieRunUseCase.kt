...
class GetAllTrackingSortedByCalorieRunUseCase(private val trackingRepository: TrackingRepository) {

    suspend operator fun invoke(): LiveData<List<TrackingHistory>> {
        return trackingRepository.getAllRunsSortedByCaloriesBurned()
    }
}